...
| Work history | `EXPERIENCE` in `components/Experience.tsx` — array order is display order; `url` is optional; a point can be `{ lead, text }` to bold a stage label ahead of the bullet, as Merge's are |
