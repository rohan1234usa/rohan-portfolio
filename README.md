...
- **SceneSense** *(Python, Imentiv API, Speech-to-Text, Embeddings)* — an AI scene partner for actors: it scores a take against the script's own emotional arc and separates an improvised line from a forgotten one. Imentiv AI · 2026, in design.
- **Pitch Coach** *(Python, FastAPI, Next.js, PostgreSQL, AWS S3, Gemini, Imentiv API)* — delivery coach for sales reps, built measurement-first: the pipeline computes the evidence and the AI only explains it. One FastAPI pipeline serves three products. Imentiv AI · 2026, pre-launch.
