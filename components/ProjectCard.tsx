...
export const ProjectCard = ({ project }: { project: ProjectProps }) => (
    <a
        ...
        className="block h-full outline-none focus-visible:ring-2 focus-visible:ring-brand-purple focus-visible:ring-offset-2 rounded-sm"
    >
        <motion.div
            whileHover={{ y: -3 }}
        ...
        >
            ...
            <div className="flex flex-wrap gap-2 mt-auto">
                {project.tech.map((t: string, i: number) => (
                    <span
                        key={i}
                        className="px-3 py-1 bg-brand-charcoal/5 text-brand-charcoal/70 text-xs font-medium tracking-wide rounded-sm border border-transparent group-hover:border-brand-purple/10 transition-colors"
                    >
                        {t}
                    </span>
                ))}
